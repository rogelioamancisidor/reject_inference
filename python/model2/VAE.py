...
class VAE(object):
    ...
    def __init__(self, xdim,num_classes,args):
        ...
        # p(x|z)
        input_to_dec = T.concatenate([self.enc_mlp.out,self.y],axis=1)                              
        #input_to_dec = self.enc_mlp.out
        if self.dec == 'bernoulli':
	        self.dec_mlp = BernoulliMLP(input_to_dec, self.zdim+self.num_classes, self.hdim_dec, self.xdim,nlayers=self.nlayers_dec, y=self.x)
        elif self.dec == 'gaussian':
	        self.dec_mlp = GaussianMLP(input_to_dec, self.zdim+self.num_classes, self.hdim_dec, self.xdim,nlayers=self.nlayers_dec, y=self.x)
        else:

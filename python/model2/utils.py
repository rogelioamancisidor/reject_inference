...
def equally_number(X,y):
    ...
    min_rows = np.min((zeros.shape[0],ones.shape[0],twos.shape[0],threes.shape[0],fours.shape[0],
                       fives.shape[0],six.shape[0],sevens.shape[0],eights.shape[0],nines.shape[0]))

    balanced_X = np.r_[zeros[0:min_rows,:],ones[0:min_rows,:],twos[0:min_rows,:],threes[0:min_rows,:],fours[0:min_rows,:],
                       fives[0:min_rows,:],six[0:min_rows,:],sevens[0:min_rows,:],eights[0:min_rows,:],nines[0:min_rows,:]]

    np.random.shuffle(balanced_X)
    y = balanced_X[:,0:10]
    X = balanced_X[:,10:794]
    return X,y
...
def load_dataset(dset='paper',scaler = True, num_rej = 35000, num_bads=4570):
    """setp 1 load data"""
    #the data as in the paper is float64 and not float32
    import gzip, cPickle
    ...
    data = (X_tr_a,y_tr_a,X_tr_r,X_te_r,X_te_a,y_te_a,X_cal,y_cal)
